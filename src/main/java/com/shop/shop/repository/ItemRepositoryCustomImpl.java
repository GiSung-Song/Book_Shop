...
public class ItemRepositoryCustomImpl implements ItemRepositoryCustom {
    ...
    private JPAQueryFactory queryFactory; //동적 쿼리 생성을 위해 JPAQueryFactory 사용

    public ItemRepositoryCustomImpl(EntityManager em) {
        this.queryFactory = new JPAQueryFactory(em);
    }

    // 상품 판매 상태 조건이 null이 아니면 판매중 or 품절 상태라면 해당 조건의 상품 조회
    ...
    private BooleanExpression regDtsAfter(String searchDateType) {
        ...
        //dateTime을 주어진 조건에 맞게 수정하여 그 이후에 등록된 item만 조회
   ...
   @Override
    public Page<Item> getAdminItemPage(ItemSearchDto itemSearchDto, Pageable pageable) {
       QueryResults<Item> results = queryFactory.selectFrom(QItem.item)
               ...
               .offset(pageable.getOffset()) //데이터를 가지고 올 시작 인데스를 지정
